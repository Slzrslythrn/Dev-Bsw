...
var map = L.map('map', {
  //center: [-6.3972, 106.8077],
  center: [-6.5950181, 106.7218511], // [41.5, -72.7] for Connecticut; [41.76, -72.67] for Hartford county or city
  zoom: 13, // zoom 9 for Connecticut; 10 for Hartford county, 12 for Hartford city
  zoomControl: true, // add later to reposition
...
L.control.geocoder('f23b3ba2e02ba8').addTo(map);
...
controlLayers.addBaseLayer(lightAll, 'OpenStreet Map');

// var lightAll = new L.tileLayer('https://c.tile.openstreetmap.org/{z}/{x}/{y}.png', {

var lightAll = new L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, &copy; <a href="http://cartodb.com/attributions">CartoDB</a>'
}).addTo(map); // adds layer by default
controlLayers.addBaseLayer(lightAll, 'OpenStreet Map 2');


// var lightAll = new L.tileLayer('https://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{x}/{y}', {
//     attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, &copy; <a href="http://cartodb.com/attributions">CartoDB</a>'
// }).addTo(map); // adds layer by default
// controlLayers.addBaseLayer(lightAll, 'OpenStreet Map 3');
...
var Esri_WorldImagery = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}', {
   attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'
});
controlLayers.addBaseLayer(Esri_WorldImagery, 'Esri World Imagery2');
...
L.Control.Watermark = L.Control.extend({
    onAdd: function(map) {
        ...
        img.src = 'src/Logo-friendly-city2.jpg';
...
// $.getJSON("src/krl.geojson", function (data){
//   var geoJsonLayer = L.geoJson(data, {
//     style: function (feature) {
//       return {
//         'color': 'purple',
...
//     },
//     onEachFeature: function( feature, layer) {
//       layer.bindPopup(feature.properties.Location) // change to match your geojson property labels
//     }
//   });  // insert ".addTo(map)" to display layer by default
//   controlLayers.addOverlay(geoJsonLayer, 'Stasiun KRL');  // insert your 'Title' to add to legend
// });


$.getJSON("src/krl.geojson", function (data){
  var iconStyle = L.icon({
    iconUrl: "src/com.gobang.kcjtrain.png",
    iconRetinaUrl: 'src/com.gobang.kcjtrain.png',
    iconSize: [30, 30]
  });
  var geoJsonLayer = L.geoJson(data, {
    pointToLayer: function( feature, latlng) {
  ...
  controlLayers.addOverlay(geoJsonLayer, 'Stasiun KRL');
});

$.getJSON("src/bogor.geojson", function (data){
  // var iconStyle = L.icon({
  //   // iconUrl: "src/com.gobang.kcjtrain.png",
  //   // iconRetinaUrl: 'src/com.gobang.kcjtrain.png',
  //   iconSize: [30, 30]
  ...
  controlLayers.addOverlay(geoJsonLayer, 'Bogor');
});

$.getJSON("src/bogorutara.geojson", function (data){
  // var iconStyle = L.icon({
  //   // iconUrl: "src/com.gobang.kcjtrain.png",
  //   // iconRetinaUrl: 'src/com.gobang.kcjtrain.png',
  //   iconSize: [30, 30]
  ...
  controlLayers.addOverlay(geoJsonLayer, 'Bogor Utara');
});
